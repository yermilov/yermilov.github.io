...
But distribution that we've just download uses version 1.8 of Gradle wrapper (for the moment of writting this article 4.0 is actual version).
...
To prove it, in the following sections I will demonstrate simple examples how you can put pieces of Groovy code pretty much everywhere in your site sources.

=== SiteConfig

As you already saw, __SiteConfig.groovy__ file contains site configuration.
...
Last but not least, you can use special ``commands`` object to create custom commands for grain cli.
